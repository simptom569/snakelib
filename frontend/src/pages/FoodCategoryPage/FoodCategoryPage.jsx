...
function FoodCategoryPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/v1/food-categories/?search=${categorySlug}`).then(r => r.json()),
      fetch(`/api/v1/foods/?page_size=100`).then(r => r.json()),
    ]).then(([catsData, foodData]) => {
      const cats = catsData.results ?? catsData;
      const cat = cats.find(c => c.slug === categorySlug);
      if (!cat) { setNotFound(true); setLoading(false); return; }
      setCategory(cat);
      const all = foodData.results ?? foodData;
      setFoods(all.filter(f => f.category?.id === cat.id));
      setLoading(false);
    }).catch(() => { setNotFound(true); setLoading(false); });
  }, [categorySlug]);

  const hasMixed = useMemo(() => {
    const types = new Set(foods.map(f => f.is_frozen));
    return types.size > 1;
  }, [foods]);
  ...
  return (
    <Page>
      ...
      <PageHeader>
        ...
        {category.description && <PageSubtitle>{category.description}</PageSubtitle>}
      </PageHeader>
      ...
      <Layout>
        ...
        <Content>
          <Toolbar>
            <ResultCount>
              Найдено: <strong>{result.length}</strong> {pluralFood(result.length)}
            </ResultCount>
            <SortWrap>
